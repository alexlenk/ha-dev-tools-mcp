...
class HAConfigurationManager:
    ...
    async def add_instance(self, instance: HAInstance) -> None:
        ...
        if instance.connection_type.value == "local":
            connection = LocalHAConnection(instance.id, instance.connection_config.url)
            ...
            connection = HAAPIConnection(
                instance.id, instance.connection_config.url, access_token
            )
        else:
            raise ConfigError(
    ...
    async def _validate_automation_schema(
        self, automations: any, result: ValidationResult
    ) -> None:
    ...
    async def switch_instance(self, instance_id: str) -> None:
        ...
        if instance_id not in self._instances:
            raise ConfigError(f"Instance {instance_id} not found", "INSTANCE_NOT_FOUND")
    ...
    async def _get_file_stats(
        ...
        try:
            ...
            return {"last_modified": datetime.now(), "size": 0}
        except Exception:
    ...
    async def close(self) -> None:
        ...
        for connection in self._connections.values():
            try:
                if hasattr(connection, "disconnect") and callable(
                    getattr(connection, "disconnect", None)
                ):
                    await connection.disconnect()
                elif hasattr(connection, "close") and callable(
