...
def validate_file_path(file_path: str) -> None:
    ...
    if ".." in file_path:
        raise ValidationError(
            "file_path cannot contain path traversal sequences (..)", "file_path"
        )

    # Check for absolute paths
    if file_path.startswith("/"):
        raise ValidationError("file_path cannot be an absolute path", "file_path")

    # Check for null bytes and other dangerous characters
    if "\x00" in file_path:
        raise ValidationError("file_path cannot contain null bytes", "file_path")
...
def validate_positive_integer(
    value: Any, parameter_name: str, min_value: int = 1, max_value: Optional[int] = None
) -> None:
    ...
    if isinstance(value, bool):
        raise ValidationError(
            f"{parameter_name} must be an integer, got bool", parameter_name
        )
    ...
    if max_value is not None and value > max_value:
        raise ValidationError(
            f"{parameter_name} must be at most {max_value}, got {value}", parameter_name
        )
...
def validate_required_parameter(value: Any, parameter_name: str) -> None:
    ...
    if value is None:
        raise ValidationError(
            f"Required parameter '{parameter_name}' is missing", parameter_name
        )
...
def validate_template(template: str) -> None:
    ...
    if not isinstance(template, str):
        raise ValidationError(
            f"template must be a string, got {type(template).__name__}", "template"
        )
