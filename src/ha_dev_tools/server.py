...
async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    try:
        ...
        if name == "list_config_files":
            ...
            return [
                TextContent(type="text", text=json.dumps({"files": files}, indent=2))
            ]
            ...
            if (
                validate_before_write is not True
            ...
            if len(file_paths) > 20:
                raise ValidationError(
                    "batch_get_metadata supports maximum 20 files", "file_paths"
                )

            metadata_list = await api_client.batch_get_metadata(file_paths)
            return [
                TextContent(
                    type="text", text=json.dumps({"metadata": metadata_list}, indent=2)
                )
            ]
            ...
            return [TextContent(type="text", text=json.dumps({"logs": logs}, indent=2))]
        ...
        # Service and Control Tools (Official HA API)
            ...
            result = await api_client.render_template(
                template, validate_entities=validate_entities
            )
            ...
            response = {"valid": True, "message": "Template syntax is valid"}
        ...
        # Diagnostics Tools (Official HA API)
            ...
            return [TextContent(type="text", text=error_log)]
            ...
            return [TextContent(type="text", text=json.dumps(logbook, indent=2))]
