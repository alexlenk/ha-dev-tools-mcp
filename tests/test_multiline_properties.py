...
class TestMultilineContextInclusion:
    ...
    def test_context_includes_surrounding_lines(self, num_lines, error_line_offset):
        ...
        try:
            ...
            assert (
                len(excerpt) >= 3
            ), "Should include at least error line and some context"
            assert (
                len(excerpt) <= 5
            ), "Should include at most 2 before + error + 2 after"
...
class TestIndentationHandling:
    ...
    @given(num_lines=st.integers(min_value=3, max_value=8))
    @settings(
        max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
...
class TestYAMLMultilineHandling:
    ...
    @given(num_content_lines=st.integers(min_value=2, max_value=5))
    @settings(
        max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
    ...
    @given(num_lines=st.integers(min_value=2, max_value=5))
    @settings(
        max_examples=100, suppress_health_check=[HealthCheck.function_scoped_fixture]
    )
