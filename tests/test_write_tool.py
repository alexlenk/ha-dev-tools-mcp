    ...
    async def test_write_file_with_expected_hash(self):
        ...
        with aioresponses() as m:
            ...
            result = await client.write_file(
                "automations.yaml", file_content, expected_hash=expected_hash
            )
  ...
  - alias: Test
    [invalid: yaml: syntax"""
        ...
        with aioresponses() as m:
            m.put(
                "http://ha.local:8123/api/management/files/automations.yaml",
                status=400,
                payload={"message": "YAML validation failed: Invalid syntax at line 3"},
            )
    ...
    async def test_write_file_access_denied(self):
        ...
        with aioresponses() as m:
            m.put(
                ...
                status=403,
                payload={"message": "Access denied: File not in allowed_paths"},
            )
    ...
    async def test_write_file_without_validation(self):
        ...
        with aioresponses() as m:
            ...
            result = await client.write_file(
                "test.txt", file_content, validate_before_write=False
            )
