...
def mock_ha_api_with_errors():
    ...
    api.read_config_file = AsyncMock(
        return_value={"content": "test content", "metadata": {"truncated": False}}
    )

    api.get_file_metadata = AsyncMock(
        return_value={"path": "test.yaml", "exists": True}
    )
...
class TestErrorScenarios:
    ...
    async def test_partial_chunk_failure(self, mock_ha_api_with_errors):
        ...
        assert (
            "incomplete" in error_message.lower() or "failed" in error_message.lower()
        )
    ...
    async def test_error_message_clarity(self):
        ...
        for scenario in error_scenarios:
            ...
            has_keywords = any(
                keyword in message_lower for keyword in scenario["expected_keywords"]
            )

            assert (
                has_keywords
            ), f"Error message missing keywords for: {scenario['error']}"
