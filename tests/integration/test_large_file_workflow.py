...
class TestLargeFileWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_large_file_workflow(
        self, mock_ha_api_chunked, large_file_content
    ):
        """
        ...
        response = await mock_ha_api_chunked.read_config_file(
            filename, offset=0, limit=chunk_size
        )
        ...
        ), "File should be larger than chunk size"
        ...
        while metadata["has_more"]:
            response = await mock_ha_api_chunked.read_config_file(
                filename, offset=offset, limit=chunk_size
            )
    ...
    async def test_truncation_detection(self, mock_ha_api_chunked, large_file_content):
        ...
        response = await mock_ha_api_chunked.read_config_file(
            "large_file.yaml", offset=0, limit=50000
        )
    ...
    async def test_chunk_assembly(self, mock_ha_api_chunked, large_file_content):
        ...
        while has_more:
            response = await mock_ha_api_chunked.read_config_file(
                "test.yaml", offset=offset, limit=chunk_size
            )
    ...
    async def test_hash_verification(self, mock_ha_api_chunked, large_file_content):
        ...
        response = await mock_ha_api_chunked.read_config_file(
            "test.yaml", offset=0, limit=50000
        )
        ...
        while has_more:
            response = await mock_ha_api_chunked.read_config_file(
                "test.yaml", offset=offset, limit=50000
            )
            chunks.append(response["content"])
    ...
    async def test_small_file_no_chunking(self, mock_ha_api_chunked):
        ...
        response = await mock_ha_api_chunked.read_config_file(
            "small.yaml", offset=0, limit=100000
        )
    ...
    @pytest.mark.asyncio
    async def test_chunk_count_calculation(
        self, mock_ha_api_chunked, large_file_content
    ):
        """
...
"""
        ...
        while has_more:
            response = await mock_ha_api_chunked.read_config_file(
                "packages/emhass.yaml", offset=offset, limit=chunk_size
            )
            chunks.append(response["content"])
            offset += response["metadata"]["returned_size"]
            has_more = response["metadata"]["has_more"]
    ...
    @pytest.mark.asyncio
    async def test_metadata_consistency_across_chunks(
        self, mock_ha_api_chunked, large_file_content
    ):
        """
