...
def mock_ha_api():
    ...
    api.write_config_file = AsyncMock(
        return_value={
            "success": True,
            "path": "automations.yaml",
...
class TestUploadWorkflow:
    ...
    async def test_complete_upload_workflow(self, mock_ha_api, valid_yaml_content):
        ...
        write_result = await mock_ha_api.write_config_file(
            filename, valid_yaml_content, expected_hash=expected_hash
        )
        ...
        assert (
            write_result["new_hash"] != expected_hash
        ), "Hash should change after upload"
        assert write_result["message"] == "File written successfully"

    @pytest.mark.asyncio
    async def test_yaml_validation_before_upload(
        self, mock_ha_api, invalid_yaml_content
    ):
        """
    ...
    @pytest.mark.asyncio
    async def test_conflict_checking_with_expected_hash(
        self, mock_ha_api, valid_yaml_content
    ):
        """
    ...
    async def test_upload_confirmation(self, mock_ha_api, valid_yaml_content):
        ...
        result = await mock_ha_api.write_config_file(
            filename, valid_yaml_content, expected_hash=metadata["hash"]
        )
    ...
    async def test_conflict_detection(self, mock_ha_api, valid_yaml_content):
        ...
        mock_ha_api.write_config_file = AsyncMock(
            return_value={
                "success": False,
        ...
        result = await mock_ha_api.write_config_file(
            filename, valid_yaml_content, expected_hash=wrong_hash
        )
    ...
    async def test_upload_new_file(self, mock_ha_api, valid_yaml_content):
        ...
        mock_ha_api.get_file_metadata = AsyncMock(
            return_value={"path": filename, "exists": False}
        )
        ...
        result = await mock_ha_api.write_config_file(filename, valid_yaml_content)
