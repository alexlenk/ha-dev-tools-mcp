...
def file_metadata_strategy(draw, path=None, hash_value=None, timestamp=None):
    ...
    return FileMetadata(
        path=path, content_hash=hash_value, modified_at=timestamp, size=size
    )
...
@given(local_meta=file_metadata_strategy(), remote_meta=file_metadata_strategy())
...
def test_property_no_conflict_when_hashes_match(path, hash_value, timestamp):
    ...
    local_meta = FileMetadata(
        path=path, content_hash=hash_value, modified_at=timestamp_str, size=1000
    )
...
def test_property_remote_newer_detection(path, base_time):
    ...
    local_meta = FileMetadata(
        path=path, content_hash="a" * 64, modified_at=local_time, size=1000
    )
...
@given(content=yaml_content_strategy())
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@given(local_content=yaml_content_strategy(), remote_content=yaml_content_strategy())
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@given(
    base_content=yaml_content_strategy(), modification=st.text(min_size=1, max_size=50)
)
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@given(content=yaml_content_strategy(), path=st.text(min_size=1, max_size=50))
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
@given(local_meta=file_metadata_strategy())
@settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
