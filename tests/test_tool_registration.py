...
class TestToolRegistration:
    ...
    async def test_file_access_tools_schemas(self):
        ...
        assert (
            "required" not in list_files.inputSchema
            or "directory" not in list_files.inputSchema.get("required", [])
        )
    ...
    async def test_entity_state_tools_schemas(self):
        ...
        assert (
            "required" not in get_states.inputSchema
            or "entity_id" not in get_states.inputSchema.get("required", [])
        )
        ...
        assert (
            "required" not in get_history.inputSchema
            or len(get_history.inputSchema.get("required", [])) == 0
        )
    ...
    async def test_service_control_tools_schemas(self):
        ...
        assert not render_template.inputSchema["properties"]["validate_entities"].get(
            "default"
        )
        ...
        # Verify validate_entities has default value
        assert not validate_template.inputSchema["properties"]["validate_entities"].get(
            "default"
        )
    ...
    async def test_tool_descriptions_are_meaningful(self):
        ...
        for tool in tools:
            ...
            assert any(
                word in tool.description.lower() for word in keywords
