...
from ha_config_manager.config import ConfigError
...
class TestServerStartup:
    ...
    async def test_server_fails_with_missing_ha_url(self):
        ...
        from ha_config_manager.server import main
    ...
    def test_main_module_entry_point_handles_keyboard_interrupt(self):
        ...
        from ha_config_manager.__main__ import run
        
        # Mock asyncio.run to raise KeyboardInterrupt
        with patch('ha_config_manager.__main__.asyncio.run', side_effect=KeyboardInterrupt):
    ...
    def test_main_module_entry_point_handles_exceptions(self):
        ...
        with patch('ha_config_manager.__main__.asyncio.run', side_effect=RuntimeError("Test error")):
            with pytest.raises(SystemExit) as exc_info:
                run()
    ...
    def test_main_module_can_be_run_as_module(self):
        """Test that the package can be run as a module (python -m ha_config_manager)."""
        # This test verifies the __main__.py structure is correct
        import importlib.util
        
        # Find the __main__.py module
        spec = importlib.util.find_spec('ha_config_manager.__main__')
