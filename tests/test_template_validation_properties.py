...
class TestTemplateValidationProperties:
    ...
    def test_property_1_entity_reference_extraction_completeness(
        ...
        **Validates: Requirements 2.1**
        ...
        if len(entity_ids) > 1:
            template_parts.append(f"{{{{ state_attr('{entity_ids[1]}', 'temperature') }}}}")
        ...
        assert result == sorted(result), \
            f"Result should be sorted: {result}"
    ...
    def test_property_1_mixed_patterns_single_entity(self):
        ...
        **Validates: Requirements 2.1**
        ...
        assert result == [entity_id], \
            f"Entity should appear only once. Got: {result}"
    ...
    def test_property_1_double_quotes(self):
        ...
        **Validates: Requirements 2.1**
        ...
        assert result_double == [entity_id], \
        ...
        **Validates: Requirements 2.2, 2.3, 2.4**
        ...
        with aioresponses() as mock:
            ...
            try:
                ...
                assert set(existing) == expected_existing, \
                ...
                assert existing == existing_order, "Existing entities should maintain input order"
                assert missing == missing_order, "Missing entities should maintain input order"
                
            except HAAPIError as e:
                pytest.fail(f"Entity validation should not raise HAAPIError for valid API response: {e}")
    ...
    @given(entity_id_list())
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=50)
    async def test_property_2_all_entities_exist(
        self,
        entity_ids: List[str]
    ):
        """
        ...
        **Validates: Requirements 2.2, 2.3, 2.4**
        ...
        with aioresponses() as mock:
            ...
            try:
                ...
                assert set(existing) == set(entity_ids), "All entities should be in existing list"
    ...
    @given(entity_id_list())
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=50)
    async def test_property_2_no_entities_exist(
        self,
        entity_ids: List[str]
    ):
        """
        ...
        **Validates: Requirements 2.2, 2.3, 2.4**
        ...
        with aioresponses() as mock:
            ...
            try:
                ...
                assert set(missing) == set(entity_ids), "All entities should be in missing list"
    ...
    def test_property_4_syntax_validation_without_execution(
        ...
        **Validates: Requirements 7.2**
        ...
        service_template = f"{{{{ states('sensor.test') }}}} {{{{ {template_content} }}}}"
        ...
        assert error is None or isinstance(error, dict), "Second return value should be None or dict"
        ...
        for valid_template in valid_templates:
            ...
            assert error is None, f"Valid template should have no error: {valid_template}"
        ...
        for invalid_template in invalid_templates:
            ...
            assert is_valid is False, f"Invalid template should fail: {invalid_template}"
            assert error is not None, f"Invalid template should have error: {invalid_template}"
            assert "error_type" in error, f"Error should have error_type: {invalid_template}"
        ...
        **Validates: Requirements 7.2**
        ...
        for template in side_effect_templates:
            ...
            assert error is None, \
                f"Valid template should have no error: {template}"
        ...
        **Validates: Requirements 7.2**
        ...
        for template, expected_error_type in syntax_error_cases:
            ...
            assert is_valid is False, \
                f"Syntax error should be detected: {template}"
            
            # Property 2: Should return error dict
            assert error is not None, \
                f"Error dict should be returned: {template}"
            
            # Property 3: Error should have required fields
            assert "error_type" in error, \
                f"Error should have error_type: {template}"
            assert "message" in error, \
                f"Error should have message: {template}"
            
            # Property 4: Error type should match expected
            assert expected_error_type in error["error_type"], \
        ...
        **Validates: Requirements 7.2**
        ...
        for template in invalid_multiline_templates:
            ...
            if "line" in error and error["line"] is not None:
                assert error["line"] > 0, \
                    f"Line number should be positive: {template}"
    
    @given(st.text(min_size=1, max_size=200))
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=100)
    def test_property_5_error_structure_completeness(
        self,
        template_content: str
    ):
        """
        ...
        **Validates: Requirements 1.1, 1.2, 1.3, 1.4, 6.1, 6.2, 6.3, 6.4, 6.5, 6.6**
        ...
        for template in invalid_templates:
            ...
            try:
                ...
                assert len(error_dict["error_type"]) > 0, \
                ...
                if "line" in error_dict and error_dict["line"] is not None:
                    assert isinstance(error_dict["line"], int), \
                ...
                try:
                    ...
                    assert "error_type" in reconstructed, "Reconstructed dict must have error_type"
                    assert "message" in reconstructed, "Reconstructed dict must have message"
                    assert reconstructed["error_type"] == error_dict["error_type"], \
                        "error_type must match"
                    assert reconstructed["message"] == error_dict["message"], \
                        "message must match"
                    
                except Exception as e:
                    pytest.fail(f"Error dict should be convertible to TemplateError: {e}")
...
{{ states('sensor.pressure') }}"""
        ...
        try:
            ...
            assert "template_excerpt" in error_dict, "Should have template_excerpt field"
            assert error_dict["template_excerpt"] is not None, "template_excerpt should not be None"
            assert isinstance(error_dict["template_excerpt"], list), "template_excerpt should be list"
            assert len(error_dict["template_excerpt"]) > 0, "template_excerpt should not be empty"
            
            # Property 5: Template excerpt should contain error marker
            has_error_marker = any("<-- ERROR" in line for line in error_dict["template_excerpt"])
    ...
    def test_property_5_error_without_line_info(self):
        ...
        **Validates: Requirements 1.1, 1.2, 1.3, 1.4, 6.1, 6.2, 6.3, 6.4, 6.5, 6.6**
        ...
        assert error_dict["message"] == "Generic error without line information", \
        ...
        if "line" in error_dict:
            assert error_dict["line"] is None or isinstance(error_dict["line"], int), \
                "line should be None or int"
        
        if "context" in error_dict:
            assert error_dict["context"] is None or isinstance(error_dict["context"], str), \
    ...
    def test_property_5_single_line_template_error(self):
        ...
        **Validates: Requirements 1.1, 1.2, 1.3, 1.4, 6.1, 6.2, 6.3, 6.4, 6.5, 6.6**
        ...
        try:
            ...
            assert "context" in error_dict, "Should have context field"
            assert error_dict["context"] == template, "Context should be the template line"
...
Line 5: {{ states('sensor.motion') }}"""
        ...
        **Validates: Requirements 1.1, 1.2, 1.3, 1.4, 6.1, 6.2, 6.3, 6.4, 6.5, 6.6**
        ...
        for template in test_cases:
            try:
                ...
                assert reconstructed_dict["error_type"] == error_dict["error_type"], \
                    "error_type must be preserved"
                assert reconstructed_dict["message"] == error_dict["message"], \
                ...
                for field in ["line", "column", "context", "template_excerpt"]:
                    if field in error_dict and error_dict[field] is not None:
                        assert field in reconstructed_dict, \
                            f"{field} should be in reconstructed dict"
                        assert reconstructed_dict[field] == error_dict[field], \
                ...
                for field in ["line", "column", "context", "template_excerpt"]:
                    if field in reconstructed_dict:
                        assert reconstructed_dict[field] is not None, \
    ...
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture], max_examples=100)
    def test_property_6_undefined_reference_error_reporting(
        self,
        template: str
    ):
        """
        ...
        **Validates: Requirements 1.5, 3.3, 3.4**
        ...
        with aioresponses() as mock:
            ...
            mock.post(
                f"{client.base_url}/api/template",
                status=400,
                body=error_message
            )
            ...
            try:
                ...
                if undefined_name:
                    assert undefined_name in e.message, \
                ...
                assert "undefined" in e.message.lower() or "not defined" in e.message.lower(), \
