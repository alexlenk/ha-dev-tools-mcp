...
def entity_id_strategy(draw):
    ...
    domain = draw(
        st.sampled_from(["sensor", "light", "switch", "binary_sensor", "climate"])
    )
    name = draw(
        st.text(
            alphabet="abcdefghijklmnopqrstuvwxyz0123456789_", min_size=3, max_size=15
        )
    )
...
async def test_property_3_validation_before_rendering(data):
    ...
    if len(unique_entities) == 1:
        ...
        num_missing = data.draw(
            st.integers(min_value=1, max_value=len(unique_entities))
        )
        missing_entities = data.draw(
            st.lists(
    ...
    client = HAAPIClient(base_url="http://localhost:8123", access_token="test_token")

    try:
        with aioresponses() as m:
            # Mock /api/states endpoint
            m.get("http://localhost:8123/api/states", payload=mock_states, status=200)

            # Mock /api/template endpoint (successful rendering)
            m.post(
                "http://localhost:8123/api/template", body=rendered_output, status=200
            )
            ...
            assert isinstance(
                result, dict
            ), "Result should be a dict when warnings are present"
            ...
            for missing_entity in missing_entities:
                assert (
                    missing_entity in warnings
                ), f"Warning should mention missing entity {missing_entity}"
    finally:
        await client.close()
...
async def test_property_8_error_information_preservation(error_type, error_message):
    ...
    try:
        with aioresponses() as m:
            ...
            m.post(
                "http://localhost:8123/api/template", payload=error_response, status=400
            )
            ...
            assert (
                error_message in error_str or "message" in error_str.lower()
