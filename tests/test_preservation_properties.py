...
class TestLocalFileOperationsPreservation:
    ...
    @given(
        directory=st.sampled_from(
            ["src", "tests", "docs", "node_modules", ".git", "build"]
        )
    )
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
class TestExplicitLocalRequestsPreservation:
    ...
    @given(
        ...
        local_keyword=st.sampled_from(
            ["local", "locally", "on my machine", "in this directory"]
        ),
    )
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    ...
    @given(
        ...
        ha_file=st.sampled_from(["configuration.yaml", "automations.yaml"]),
    )
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
...
class TestSmallFileRetrievalPreservation:
    ...
    @given(file_size=st.integers(min_value=1, max_value=9999))
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    def test_small_files_no_truncation_metadata(self, file_size):
        ...
        expected_response = {
            ...
            "metadata": {"size": file_size, "truncated": False, "has_more": False},
        }
...
class TestErrorHandlingPreservation:
    ...
    def _is_clear_error_message(self, message: str) -> bool:
        ...
        return len(message) > 10 and not message.isdigit() and " " in message
...
class TestPowerInstallationFallback:
    ...
    @given(ha_file=st.sampled_from(["configuration.yaml", "automations.yaml"]))
    @settings(suppress_health_check=[HealthCheck.function_scoped_fixture])
    ...
    def _should_use_local_tools_fallback(
        self, request: str, power_installed: bool, ha_configured: bool = True
    ) -> bool:
