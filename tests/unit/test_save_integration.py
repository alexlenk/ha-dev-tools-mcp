...
async def test_read_config_file_with_save_local_true(manager, mock_connection):
    ...
    with patch.object(manager.file_saver, 'save_file', new_callable=AsyncMock) as mock_save:
...
@pytest.mark.asyncio
async def test_error_when_save_local_and_both_pagination_params(manager, mock_connection):
...
async def test_security_error_handling(manager, mock_connection):
    ...
    with patch.object(manager.file_saver, 'save_file', new_callable=AsyncMock) as mock_save:
        ...
        assert "Security error" in str(exc_info.value) or "security" in str(exc_info.value).lower()
...
async def test_io_error_handling(manager, mock_connection):
    ...
    with patch.object(manager.file_saver, 'save_file', new_callable=AsyncMock) as mock_save:
        ...
        assert "Failed to save" in str(exc_info.value) or "save" in str(exc_info.value).lower()
