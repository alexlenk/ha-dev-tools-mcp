...
def test_workflow_state_persistence_property(
    temp_workspace, workflow_type, files, total_steps
):
    ...
    workflow = manager.create_workflow(
        workflow_type=workflow_type, files=files, total_steps=total_steps
    )
    ...
    assert (
        resumed_workflow is not None
    ), "Workflow should be loadable after interruption"
    assert (
        resumed_workflow.workflow_id == original_id
    ), "Workflow ID should be preserved"
    assert (
        resumed_workflow.workflow_type == original_type
    ), "Workflow type should be preserved"
    assert (
        len(resumed_workflow.files) == original_files_count
    ), "File count should be preserved"
    assert (
        resumed_workflow.total_steps == original_total_steps
    ), "Total steps should be preserved"
    assert (
        resumed_workflow.status == WorkflowStatus.IN_PROGRESS
    ), "Status should be preserved"

    # Verify file details are preserved
    for original_file, resumed_file in zip(workflow.files, resumed_workflow.files):
        assert (
            resumed_file.local_path == original_file.local_path
        ), "Local path should be preserved"
        assert (
            resumed_file.remote_path == original_file.remote_path
        ), "Remote path should be preserved"
        assert (
            resumed_file.downloaded_at == original_file.downloaded_at
        ), "Download timestamp should be preserved"
        assert (
            resumed_file.remote_hash == original_file.remote_hash
        ), "Remote hash should be preserved"
        assert (
            resumed_file.status == original_file.status
        ), "File status should be preserved"
...
def test_workflow_step_update_persistence(temp_workspace, workflow_type, files, step):
    ...
    # Create workflow
    workflow = manager.create_workflow(
        workflow_type=workflow_type, files=files, total_steps=max(step, 10)
    )
...
def test_workflow_file_modification_persistence(temp_workspace, workflow_type, files):
    ...
    # Create workflow
    workflow = manager.create_workflow(
        workflow_type=workflow_type, files=files, total_steps=5
    )
    ...
    assert resumed is not None
    assert (
        resumed.files[0].status == FileStatus.MODIFIED
    ), "File modification should be persisted"
...
def test_workflow_completion_persistence(temp_workspace, workflow_type, files):
    ...
    assert resumed is not None
    assert (
        resumed.status == WorkflowStatus.COMPLETED
    ), "Workflow completion should be persisted"
