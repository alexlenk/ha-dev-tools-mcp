        ...
        with patch.object(server_module, "logger") as mock_logger:
            ...
            with aioresponses() as mock:
                ...
                if tool_name == "list_config_files":
                    ...
                    mock.get(
                        "http://test.local:8123/api/states", status=200, payload=[]
                    )
                    ...
                    mock.get(
                        "http://test.local:8123/api/events", status=200, payload=[]
                    )
                elif tool_name == "list_services":
                    mock.get(
                        "http://test.local:8123/api/services", status=200, payload={}
                    )
                ...
                server_module.api_client = HAAPIClient(
                    "http://test.local:8123", "test_token"
                )
                ...
                try:
                    ...
                    tool_logged = any(tool_name in str(call) for call in info_calls)
                    ...
                    assert (
                        tool_logged
                    ), f"Tool name '{tool_name}' should be logged even on error"
            ...
            with aioresponses() as mock:
                ...
                try:
                    ...
                    await handle_call_tool(
                        "read_config_file", {"file_path": "test.yaml"}
                    )
                except Exception:
        ...
        with patch.object(server_module, "logger") as mock_logger:
            with aioresponses() as mock:
                ...
                if is_success:
                    ...
                    assert (
                        len(info_calls) > 0
                    ), "Successful operations should log at INFO level"
                else:
                    # Property: Failed operations should use ERROR level
                    error_calls = [call for call in mock_logger.error.call_args_list]
                    assert (
                        len(error_calls) > 0
                    ), "Failed operations should log at ERROR level"
        ...
        **Validates: Requirements 6.4, 10.5**
        ...
        with patch.object(server_module, "logger") as mock_logger:
            with aioresponses() as mock:
                ...
                server_module.api_client = HAAPIClient(
                    "http://test.local:8123", access_token
                )
                ...
                try:
                    await handle_call_tool(
                        "read_config_file", {"file_path": "error.yaml"}
                    )
                except Exception:
                    pass
